...
stage('Run Unit Tests') {
    steps {
        ...
        java -cp "${WORKSPACE}/test_output:${WORKSPACE}/src/main/webapp/WEB-INF/lib/*" org.junit.platform.console.ConsoleLauncher --scan-class-path > ${WORKSPACE}/test_results.log
        """
        
        script {
            def testResults = sh(script: "grep -E 'Tests run:|Failures:' ${WORKSPACE}/test_results.log", returnStdout: true).trim()
            echo "Test Summary:\n${testResults}"
        }
    }
}
